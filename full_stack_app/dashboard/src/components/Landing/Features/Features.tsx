        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <div
              ...
              className={`
                p-6 rounded-xl border border-gray-100 dark:border-gray-700
                transition-all duration-300 group cursor-pointer
                ${activeFeature === index 
                  ? 'border-indigo-400 dark:border-indigo-500 shadow-lg transform -translate-y-1' 
                  : 'hover:border-indigo-100 dark:hover:border-indigo-800'}
              `}
            >
              <div className="w-12 h-12 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 
